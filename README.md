...
    // NEW: A central function to save the cart and refresh all relevant displays
...
    // NEW: Functions to adjust quantity directly from the cart
...
            removeFromCart(productId, false); // don't show toast again
        }
        saveAndRefreshCart();
    }
    
    function removeFromCart(productId, showMsg = true) {
        const itemIndex = cart.findIndex(i => i.id === productId);
        if (itemIndex > -1) {
            const itemName = cart[itemIndex].name;
            cart.splice(itemIndex, 1);
            if(showMsg) showToast(`${itemName} removed from cart.`);
        }
        saveAndRefreshCart();
    }
...
    
    // MODIFIED: Cart display now includes quantity adjustment buttons
...
                    <button class="btn btn-secondary" style="padding: 0.2rem 0.6rem; color: #333; border-color: #ccc;" onclick="decreaseQuantity(${item.id})">-</button>
                    <span style="min-width: 20px; text-align: center; font-weight: bold;">${item.quantity}</span>
                    <button class="btn btn-secondary" style="padding: 0.2rem 0.6rem; color: #333; border-color: #ccc;" onclick="increaseQuantity(${item.id})">+</button>
                </div>
            </div>
